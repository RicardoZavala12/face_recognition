...
# Crear una carpeta para almacenar las imágenes de las personas (base de datos)
folder_path = 'base_de_datos'
if not os.path.exists(folder_path):
    os.makedirs(folder_path)

# Obtener las imágenes de las personas para crear la base de datos
...
while True:
    ...
    for face_encoding in face_encodings:
        ...
        # Si encontramos una coincidencia, obtenemos el nombre de la persona
        if True in matches:
            first_match_index = matches.index(True)
            name = person_names[first_match_index]
        ...
        # Guardar la imagen si se detecta una persona conocida
        if name != "Desconocido":
            now = datetime.now()
            date_time = now.strftime("%Y%m%d%H%M%S")
            cv2.imwrite(f'{folder_path}/{name}_{date_time}.jpg', frame)
